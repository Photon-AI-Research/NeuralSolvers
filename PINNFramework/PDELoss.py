...
class PDELossAdaptive(PDELoss):
    def __init__(self, geometry, pde, sampler, name, norm='L2', weight=1.):
        """
        Constructor of the PDE Loss for adaptive sampling

        Args:
            geometry (torch.utils.Dataset): provides the geometry of the domain
            pde (function): function that represents residual of the PDE
            sampler: represents the type of sampling to be used
            norm: Norm used for calculation PDE loss
            weight: Weighting for the loss term
        """
    ...
    def __call__(self, x: Tensor, model: Module, **kwargs):
        """
        Call function of the adaptive PDE loss. Calculates the norm of the PDE residual

        x: residual points
        model: model that predicts the solution of the PDE
        """
        x, weight = self.sampler.sample(model, self.pde)
